...
helperAttributes.oracleAggregatorAbi = [
  ...
  },
  {
    inputs: [
      ...
      { internalType: "uint256", name: "ethGasPrice", type: "uint256" },
    ],
    name: "getTokenGasPrice",
    outputs: [
      {
        internalType: "uint256",
        name: "tokenGasPriceUnadjusted",
        type: "uint256",
      },
    ],
...
helperAttributes.transferHandlerAbi = [
  {
...
helperAttributes.erc20ForwarderAbi = [
  {
    inputs: [{ internalType: "address", name: "_owner", type: "address" }],
    stateMutability: "nonpayable",
    type: "constructor",
...
helperAttributes.feeManagerAbi = [
  {
    inputs: [
...
helperAttributes.erc20Eip2612Abi = [
  {
    anonymous: false,
    inputs: [
      {
...
const buildForwardTxRequest = async (
  ...
  const feeMultiplier = await feeManager.getFeeMultiplier(
    account,
    tokenAddress
  );
  const transferHandlerGas = await erc20Forwarder.transferHandlerGas(
    tokenAddress
  );
...
const getDaiPermit = async (provider, account, daiPermitOptions) => {
  ...
  const spender =
    daiPermitOptions.spender ||
    erc20ForwarderAddressMap[daiPermitOptions.networkId];
  const expiry =
...
const getEip2612Permit = async (provider, account, permitOptions) => {
  ...
  const spender =
    permitOptions.spender || erc20ForwarderAddressMap[permitOptions.networkId];
